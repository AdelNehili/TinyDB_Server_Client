...
void student_to_str(char* buffer, const student_t* s, size_t buffer_size) {
  ...
  char my_buffer[4096];
  snprintf(my_buffer,buffer_size, "%.9u: %s %s in section %s, born on the %.2d/%.2d/%.2d \n", s->id, s->fname, s->lname, s->section, day, month + 1, year + 1900);
  strcat(buffer, my_buffer);
}

void student_nbr_to_buffer(char* buffer, size_t buffer_size,int i) {
  char my_buffer[4096];
  snprintf(my_buffer, buffer_size,"%d students selected \n",i);
  strcat(buffer, my_buffer);
}
